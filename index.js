		...
		try {
			...
			switch(command) {
				...
				case 'tagall':
					...
					teks += '\n\n'
					for (let mem of groupMembers) {
						teks += `╠➥ bora acordar 🥰 @${mem.jid.split('@')[0]}\n`
						members_id.push(mem.jid)
					}
					reply(teks)
					break
				...
				case 'ban':
					...
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = 'que pena, espero que ele se arrependa 🌹:\n'
						for (let _ of mentioned) {
							teks += `@${_.split('@')[0]}\n`
						}
						mentions(teks, mentioned, true)
						...
						mentions(`que pena, espero que ele se arrependa 🙏 : @${mentioned[0].split('@')[0]}`, mentioned, true)
						client.groupRemove(from, mentioned)
					}
					break
				case 'listadmins':
