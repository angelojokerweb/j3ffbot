	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Tchauu @${num.split('@')[0]} Volte sempre amigo 😎👋`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				...
				success: '✔️ Pronto toma isso ae ✔️',
				error: {
				...
				only: {
					...
					ownerB: '❌ Este comando só pode ser usado pelo Ângelo, idiota! ❌',
					admin: '❌ Este comando só pode ser usado por Ademiros do grupo, Membro comum! 👺❌',
			...
			switch(command) {
				...
				case 'sticker':
					...
					/*} else if ((isMedia || isQuotedImage) && colors.includes(args[0])) {
						...
						reply(`mano cadê a foto para botar o comando ${prefix}sticker`)
					}
					break
				case 'gtts':
				...
				case 'dono':
					...
					buffer = await getBuffer(`https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRG2NwRaai22LHNHN9SUL6Sp1XDR3jMbneTMw&usqp=CAU`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '*DONO: ANGELO*\n *WPP:* wa.me/+64992606313'})
					break
				case 'memeindo':
					memein = await kagApi.memeindo()
				...
				case 'setprefix':
					...
					prefix = args[0]
					reply(`Affs Claro Ângelo, ja mudei o negócio para você amor 😍🤙 : ${prefix}`)
					break
				case 'loli':
				...
				case 'hilih':
					...
					break
				case 'yt2mp3':
					if (args.length < 1) return reply('Qual url é um?')
            ...
            case 'tagall4':
			    ...
			    if (!isUser) return reply(mess.only.userB)
					...
					mentions(`╔═══✪ marcando meus amores 😳🤙 *${pushname2}* ✪══`+ teks +'╚═══〘 ÂNGELO BOTS 〙═══', members_id, true)
					break
				case 'clearall':
					...
					reply('Excluiu todo o bate-papo com sucesso :)')
					break
				case 'bc':
					if (!isOwner) return reply('Kamu siapa?')
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						reply('Transmissão para todos com sucesso')
					} else {
						for (let _ of anu) {
				...
				case 'add':
					...
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (args.length < 1) return reply('Você quer adicionar quem? o Bolsomito? 😳')
					if (args[0].startsWith('08')) return reply('Gunakan kode negara, tapi')
					try {
				...
				case 'ban':
					...
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = 'TOMOU O FAMOSO BAN DO ADM 😳🤙🔥:\n'
						for (let _ of mentioned) {
							teks += `@${_.split('@')[0]}\n`
						}
						mentions(teks, mentioned, true)
						...
						mentions(`TOMOU O FAMOSO BAN DO ADM 😳🤙 : @${mentioned[0].split('@')[0]}`, mentioned, true)
						client.groupRemove(from, mentioned)
					}
					break
				case 'listadmins':
